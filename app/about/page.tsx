...
const CTA = dynamic(
  () => import("@/modules/content/CTA").then((m) => m.CTA)
);
...
const FALLBACK_CTA = {
  ...
  description:
    "Browse our current portfolio of villas, apartments, plots and gated communities.",
...
export default async function AboutPage() {
  ...
  const teamBlock = aboutPage?.blocks.find((b) => b.type === "team");
  ...
  const storyImage = resolveMediaUrl(
    textBlock?.config.image_url ||
      textBlock?.config.featured_image_url ||
      textBlock?.config.image
  );
  ...
  return (
    <>
      ...
      <OurStory
        heading={textBlock?.config.heading}
        paragraphs={paragraphs}
        image={storyImage}
      />

      <TeamSection
        heading={teamBlock?.config.heading}
        members={teamBlock?.config.members}
      />
